...
/* ------------------------------------------------------------------ */
/*  FleetTab — wraps all Fleet content for embedding in Command Center */
/* ------------------------------------------------------------------ */
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2 text-sm text-slate-500">
          <Cpu className="w-4 h-4 text-violet-400 animate-pulse" strokeWidth={1.5} />
          Loading fleet data…
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
  ...
  if (error || !data) {
    return (
      <div className="space-y-4">
        <p className="text-red-400 text-sm">Error loading fleet data: {error ?? "No data"}</p>
        <button
          onClick={() => fetchData()}
