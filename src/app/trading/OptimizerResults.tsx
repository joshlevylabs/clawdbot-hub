                  ...
                  <div>
                    ...
                    <div className="bg-slate-800/50 rounded p-2">
                      <div className="flex items-center justify-between text-xs">
                        {strategy.versions.slice().reverse().map((version, idx) => (
...
export default function OptimizerResults() {
  ...
  const fetchAll = async () => {
    try {
      ...
      setError(null);
      const ts = Date.now();
      const [wfRes, pgRes, mcRes, calRes, verRes, stratRes] = await Promise.all([
      ...
      if (!wfRes.ok || !pgRes.ok || !mcRes.ok || !calRes.ok || !verRes.ok || !stratRes.ok) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
              ...
              Universe Optimizer — 5-Phase Quant Pipeline
            </h2>
