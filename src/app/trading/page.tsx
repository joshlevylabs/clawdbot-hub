...
function EquityCurveChart({ data }: { data: PortfolioSnapshot[] }) {
  if (data.length < 2) {
    return (
      <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700/50 text-center">
        ...
        <p className="text-slate-400">Equity curve appears after more daily snapshots</p>
      </div>
    );
  }

  const startEquity = data[0].equity;
  const startSpy = data[0].spy_baseline || data[0].spy_price || 1;

  const portfolioReturns = data.map((d) => ((d.equity - startEquity) / startEquity) * 100);
  const spyReturns = data.map((d) => {
    const spy = d.spy_price || startSpy;
    return ((spy - startSpy) / startSpy) * 100;
  });

  const maxReturn = Math.max(...portfolioReturns, ...spyReturns, 5);
  const minReturn = Math.min(...portfolioReturns, ...spyReturns, -5);
  const range = maxReturn - minReturn || 10;
  ...
  return (
    <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
      ...
      <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="w-full h-32">
        <line x1="0" y1={getY(0)} x2={chartWidth} y2={getY(0)} stroke="#475569" strokeDasharray="2,2" />
