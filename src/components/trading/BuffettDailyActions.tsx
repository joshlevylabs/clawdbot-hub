...
interface BuffettActionsData {
  date: string;
  market_assessment: string;
  pre_market_actions: BuffettAction[];
  market_hours_actions: BuffettAction[];
...
export default function BuffettDailyActions() {
  const [data, setData] = useState<BuffettActionsData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const CACHE_KEY = 'buffett-daily-actions-v1';
  const CURRENT_KNOWLEDGE = 'warren-buffett-v1-portfolio-letters';
  ...
  const fetchAnalysis = async (refresh: boolean = false) => {
    ...
    try {
      const url = `/api/trading/buffett-actions${refresh ? '?refresh=true' : ''}`;
      ...
      console.error('Failed to fetch Buffett actions:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg">
        <div className="p-6">
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-3 text-slate-400">Loading Buffett&apos;s analysis...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg">
        <div className="p-6">
          <div className="text-center py-8">
            ...
            <p className="text-slate-300">Buffett&apos;s analysis is unavailable</p>
  ...
  return (
    <div className="bg-slate-800/50 border-2 border-emerald-500/20 rounded-lg">
      <div className="p-6">
        {/* Header */}
        ...
        {data?._stale_guard && (
          <div className="p-4 bg-red-900/20 rounded-lg border border-red-700/40 flex items-start gap-3 mb-6">
            <AlertTriangle className="h-5 w-5 text-red-400 mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-sm font-medium text-red-300">Signal Data Expired</p>
              <p className="text-xs text-red-400/80 mt-1">
                Analysis was not generated because signal data is too old. Refresh signals to get Warren&apos;s assessment.
              </p>
            </div>
          </div>
        )}
        ...
        {data?.market_assessment && !data?._stale_guard && (
          <div className="p-4 bg-slate-700/30 rounded-lg border border-slate-600/30 mb-6">
            <p className="text-slate-200 text-sm leading-relaxed italic">&ldquo;{data.market_assessment}&rdquo;</p>
          </div>
        )}
                  ...
                  <div key={idx} className="p-3 bg-slate-700/20 rounded border border-slate-600/20">
                    <div className="flex items-start justify-between mb-2">
                      <span className="text-sm font-medium text-white">
                        {action.ticker && <span className="text-emerald-400">{action.ticker}: </span>}
                        {action.action}
                      </span>
                  ...
                  <div key={idx} className="p-3 bg-slate-700/20 rounded border border-slate-600/20">
                    <div className="flex items-start justify-between mb-2">
                      <span className="text-sm font-medium text-white">
                        {action.ticker && <span className="text-emerald-400">{action.ticker}: </span>}
                        {action.action}
                        {action.price_level && <span className="text-emerald-400"> @ {action.price_level}</span>}
                      </span>
                      {getPriorityBadge(action.priority)}
                    </div>
                    <p className="text-xs text-slate-400">{action.rationale}</p>
        ...
        {data?.positions_review?.length ? (
          <div className="space-y-3 mb-6">
            <h4 className="text-sm font-medium text-slate-300 flex items-center">
              ...
              Positions Review — Business Owner&apos;s Perspective
            </h4>
              ...
              {data.positions_review.map((position, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-slate-700/20 rounded border border-slate-600/20">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm font-medium text-emerald-400">{position.ticker}</span>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {data.watchlist.map((ticker, idx) => (
                  <span 
                    key={idx} 
                    className="px-2 py-1 text-xs rounded-md font-medium bg-emerald-600/20 text-emerald-300 border border-emerald-500/30"
                  >
                    {ticker}
                  </span>
                ))}
